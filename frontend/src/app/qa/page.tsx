  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-slate-900">QA & Compliance</h1>
          <p className="mt-1 text-slate-500">Automated quality scoring against compliance rules</p>
        </div>
      ...
      {isLoading ? (
        ...
        <Card>
          <CardContent className="flex flex-col items-center justify-center py-16">
            ...
            <p className="mt-4 text-lg font-medium text-slate-500">No QA scores yet</p>
          ...
          <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
            <Card>
              <CardContent className="p-4 text-center">
                <p className="text-sm text-slate-500">Avg QA Score</p>
                <p className={`mt-1 text-3xl font-bold ${avgScore >= 70 ? "text-emerald-600" : avgScore >= 50 ? "text-amber-600" : "text-red-600"}`}>
                  {avgScore}/100
                </p>
              </CardContent>
            </Card>
            <Card>
              <CardContent className="p-4 text-center">
                <p className="text-sm text-slate-500">Calls Scored</p>
                <p className="mt-1 text-3xl font-bold text-slate-900">{scores.length}</p>
              </CardContent>
            </Card>
            <Card>
              <CardContent className="p-4 text-center">
                <p className="text-sm text-slate-500">Critical Flags</p>
                <p className={`mt-1 text-3xl font-bold ${criticalCount > 0 ? "text-red-600" : "text-emerald-600"}`}>
                  {criticalCount}
                </p>
              </CardContent>
            </Card>
            <Card>
              <CardContent className="p-4 text-center">
                <p className="text-sm text-slate-500">Mandatory Failures</p>
                <p className={`mt-1 text-3xl font-bold ${mandatoryFailCount > 0 ? "text-red-600" : "text-emerald-600"}`}>
                  {mandatoryFailCount}
                </p>
              </CardContent>
            </Card>
              ...
              <Card key={score.id} className="overflow-hidden">
                  ...
                  <div className="flex items-center gap-4">
                    <div className="relative">
                      <div className={`flex h-14 w-14 items-center justify-center rounded-full text-lg font-bold text-white ${
                        score.total_score >= 70 ? "bg-emerald-500" : score.total_score >= 50 ? "bg-amber-500" : "bg-red-500"
                      }`}>
                        {Math.round(score.total_score)}
                      </div>
                    </div>
                    <div>
                      <p className="font-semibold text-slate-900">{score.original_filename}</p>
                      <p className="text-sm text-slate-500">Agent: {score.agent_name}</p>
                    </div>
                  </div>
                ...
                {expanded === score.call_id && detail?.breakdown && (
                  <div className="border-t border-slate-100 bg-slate-50 p-6 space-y-6">
                    {/* Rule results */}
                    <div>
                      <h3 className="mb-3 text-sm font-semibold text-slate-700">Compliance Rules</h3>
                      <div className="space-y-2">
                        {detail.breakdown.rule_results?.map((rule, i) => (
                          <div key={i} className="flex items-start justify-between rounded-lg border bg-white p-3">
                            <div className="flex items-start gap-3">
                              ...
                              <div>
                                ...
                                <p className="text-xs text-slate-500">{rule.evidence}</p>
                              </div>
                            </div>
                    ...
                    {detail.breakdown.flags?.length > 0 && (
                      <div>
                        <h3 className="mb-3 text-sm font-semibold text-red-700">Compliance Flags</h3>
                        <div className="space-y-2">
                          {detail.breakdown.flags.map((flag, i) => (
                            <div key={i} className={`rounded-lg border p-3 ${
                              flag.severity === "critical" ? "border-red-200 bg-red-50" :
                              flag.severity === "warning" ? "border-amber-200 bg-amber-50" : "border-blue-200 bg-blue-50"
                            }`}>
                    ...
                    {detail.breakdown.positive_notes?.length > 0 && (
                      <div>
                        <h3 className="mb-3 text-sm font-semibold text-emerald-700">Positive Notes</h3>
